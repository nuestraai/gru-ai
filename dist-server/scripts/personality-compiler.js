...
export function compilePersonality(agentId, platform) {
    ...
    if (!agent) {
        ...
        agent = registry.agents.find(a => a.agentFile && a.agentFile.replace(/\.md$/, '') === agentId);
    }
    if (!agent) {
...
function compileForCodex(agent, fields, body) {
    const lines = [];
    ...
    for (const [key, value] of Object.entries(fields)) {
        if (CLAUDE_CODE_ONLY_FIELDS.has(key))
            continue;
...
const isMain = process.argv[1] && (process.argv[1].endsWith('personality-compiler.ts') ||
